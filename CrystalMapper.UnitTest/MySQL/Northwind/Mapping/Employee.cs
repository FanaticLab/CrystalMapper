...
namespace CrystalMapper.UnitTest.MySQL.Northwind
{
    ...
    {		
        ...
        public const string PARAM_SALARY = "@Salary";	
		
        #endregion
		
		#region Queries
		
		private const string SQL_INSERT_EMPLOYEES = "INSERT INTO employees (LastName, FirstName, Title, TitleOfCourtesy, BirthDate, HireDate, Address, City, Region, PostalCode, Country, HomePhone, Extension, Photo, Notes, ReportsTo, PhotoPath, Salary) VALUES ( @LastName, @FirstName, @Title, @TitleOfCourtesy, @BirthDate, @HireDate, @Address, @City, @Region, @PostalCode, @Country, @HomePhone, @Extension, @Photo, @Notes, @ReportsTo, @PhotoPath, @Salary);"   + " SELECT LAST_INSERT_ID();" ;
		
		private const string SQL_UPDATE_EMPLOYEES = "UPDATE employees SETLastName = @LastName, FirstName = @FirstName, Title = @Title, TitleOfCourtesy = @TitleOfCourtesy, BirthDate = @BirthDate, HireDate = @HireDate, Address = @Address, City = @City, Region = @Region, PostalCode = @PostalCode, Country = @Country, HomePhone = @HomePhone, Extension = @Extension, Photo = @Photo, Notes = @Notes, ReportsTo = @ReportsTo, PhotoPath = @PhotoPath, Salary = @Salary WHERE EmployeeID = @EmployeeID";
		...
		protected float? salary = default(float?);
        ...
        [Column(COL_SALARY, PARAM_SALARY )]
        ...
        public IQueryable<Employee> EmployeeReportsTos 
        { 
            get { return this.CreateQuery<Employee>().Where(r => r.ReportsTo == EmployeeID); }
        }
        
        public IQueryable<EmployeeTerritory> EmployeeTerritories 
        { 
            get { return this.CreateQuery<EmployeeTerritory>().Where(r => r.EmployeeID == EmployeeID); }
        }
        
        public IQueryable<Order> Orders 
        { 
            get { return this.CreateQuery<Order>().Where(r => r.EmployeeID == EmployeeID); }
        }
		...
		bool IRecord.Create(DataContext dataContext)
        {
            ...
            {	
				...
				command.Parameters.Add(dataContext.CreateParameter(PARAM_NOTES, this.Notes));
				...
				command.Parameters.Add(dataContext.CreateParameter(PARAM_SALARY, DbConvert.DbValue(this.Salary)));
