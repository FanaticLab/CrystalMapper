...
namespace CrystalMapper.UnitTest.MySQL.Northwind
{
    ...
    {		
        ...
        public const string TABLE_NAME = "employee_territories";	
		...
		private const string SQL_INSERT_EMPLOYEE_TERRITORIES = "INSERT INTO employee_territories (EmployeeID, TerritoryID) VALUES ( @EmployeeID, @TerritoryID);"  ;
		
		private const string SQL_UPDATE_EMPLOYEE_TERRITORIES = "UPDATE employee_territories SET WHERE EmployeeID = @EmployeeID AND TerritoryID = @TerritoryID";
		
		private const string SQL_DELETE_EMPLOYEE_TERRITORIES = "DELETE FROM employee_territories WHERE  EmployeeID = @EmployeeID AND TerritoryID = @TerritoryID ";
        ...
        {
            using(DbCommand command  = dataContext.CreateCommand(SQL_INSERT_EMPLOYEE_TERRITORIES))
            {	
        ...
        {
            using(DbCommand command  = dataContext.CreateCommand(SQL_UPDATE_EMPLOYEE_TERRITORIES))
            {							
				command.Parameters.Add(dataContext.CreateParameter(PARAM_EMPLOYEEID, this.EmployeeID));
				command.Parameters.Add(dataContext.CreateParameter(PARAM_TERRITORYID, this.TerritoryID));
        ...
        {
            using(DbCommand command  = dataContext.CreateCommand(SQL_DELETE_EMPLOYEE_TERRITORIES))
            {							
				command.Parameters.Add(dataContext.CreateParameter(PARAM_EMPLOYEEID, this.EmployeeID));
				command.Parameters.Add(dataContext.CreateParameter(PARAM_TERRITORYID, this.TerritoryID));
                return (command.ExecuteNonQuery() == 1);
