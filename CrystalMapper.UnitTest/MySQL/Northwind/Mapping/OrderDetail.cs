...
namespace CrystalMapper.UnitTest.MySQL.Northwind
{
    ...
    {		
		...
		private const string SQL_UPDATE_ORDER_DETAILS = "UPDATE `order details` SETUnitPrice = @UnitPrice, Quantity = @Quantity, Discount = @Discount WHERE OrderID = @OrderID AND ProductID = @ProductID";
		...
		protected double discount = default(double);
        ...
        [Column(COL_DISCOUNT, PARAM_DISCOUNT, default(double))]
		...
		{       
			...
			this.discount = (double)reader[COL_DISCOUNT];
		}
