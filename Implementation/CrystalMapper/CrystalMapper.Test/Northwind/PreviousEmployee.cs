...
{
	[Table(TABLE_NAME)]
    public partial class PreviousEmployee : Entity< PreviousEmployee>
    {		
		#region Table Schema
		
        public const string TABLE_NAME = "PreviousEmployees";	
		...
		private const string SQL_INSERT_PREVIOUSEMPLOYEES = "INSERT INTO PreviousEmployees([EmployeeID],[LastName],[FirstName],[Title],[TitleOfCourtesy],[BirthDate],[HireDate],[Address],[City],[Region],[PostalCode],[Country],[HomePhone],[Extension],[Photo],[Notes],[PhotoPath]) VALUES (:EmployeeID,:LastName,:FirstName,:Title,:TitleOfCourtesy,:BirthDate,:HireDate,:Address,:City,:Region,:PostalCode,:Country,:HomePhone,:Extension,:Photo,:Notes,:PhotoPath);"  ;
		
		private const string SQL_UPDATE_PREVIOUSEMPLOYEES = "UPDATE PreviousEmployees SET [LastName] = :LastName, [FirstName] = :FirstName, [Title] = :Title, [TitleOfCourtesy] = :TitleOfCourtesy, [BirthDate] = :BirthDate, [HireDate] = :HireDate, [Address] = :Address, [City] = :City, [Region] = :Region, [PostalCode] = :PostalCode, [Country] = :Country, [HomePhone] = :HomePhone, [Extension] = :Extension, [Photo] = :Photo, [Notes] = :Notes, [PhotoPath] = :PhotoPath,  WHERE [EmployeeID] = :EmployeeID";
		
		private const string SQL_DELETE_PREVIOUSEMPLOYEES = "DELETE FROM PreviousEmployees WHERE  [EmployeeID] = :EmployeeID ";
        ...
        {
            using(DbCommand command  = dataContext.CreateCommand(SQL_INSERT_PREVIOUSEMPLOYEES))
            {	
        ...
        {
            using(DbCommand command  = dataContext.CreateCommand(SQL_UPDATE_PREVIOUSEMPLOYEES))
            {							
				command.Parameters.Add(dataContext.CreateParameter(this.EmployeeID, PARAM_EMPLOYEEID));
				command.Parameters.Add(dataContext.CreateParameter(this.LastName, PARAM_LASTNAME));
				command.Parameters.Add(dataContext.CreateParameter(this.FirstName, PARAM_FIRSTNAME));
        ...
        {
            using(DbCommand command  = dataContext.CreateCommand(SQL_DELETE_PREVIOUSEMPLOYEES))
            {							
				command.Parameters.Add(dataContext.CreateParameter(this.EmployeeID, PARAM_EMPLOYEEID));				
                return (command.ExecuteNonQuery() == 1);
            }
